...
one_year_before = datetime.now(tz=timezone.utc) - timedelta(days=365)
...
for proposal in all_proposals:
    proposal_date = parser.isoparse(proposal["date"])
    if (
        proposal_date >= one_year_before
        and proposal["dao_address"] == WAVES_DAO_ADDRESS
    ):
...
with open("woc-waves-dao-last-year-vote-percentage.json", "w", encoding="utf-8") as f:
    json.dump(result, f, indent=4)

to_be_slashed = [committer for committer in result if committer["percentage"] < 0.30]

with open("woc-waves-dao-last-year-to-be-slashed.json", "w", encoding="utf-8") as f:
...
with open("woc-waves-dao-last-year-tx-arg.json", "w", encoding="utf-8") as f:
